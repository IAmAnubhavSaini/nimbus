...
//UCHAR               FTDeviceAddress[NDIS_MAX_PHYS_ADDRESS_LENGTH];
...
--*/
{
    ...
    do
    {
		...
		//NdisMoveMemory(FTDeviceAddress, AttachParameters->CurrentMacAddress, NDIS_MAX_PHYS_ADDRESS_LENGTH);
...
--*/
{
    ...
    do
    {
		...
		while (CurrentNBL != NULL)
		{
			...
			while (CurrentNB)
			{
				//DbgPrint("Receiving NB %p\n", CurrentNB);
				FTProcessNetPacket(CurrentNB);
									//CurrentNBL);
				CurrentNB = NET_BUFFER_NEXT_NB(CurrentNB);
				//ft.CountTokenPackets++;
			}
			PrevNBL = CurrentNBL;
		...
		//if (ft.CountTokenPackets >= ft.MaxTokenSize) //rate limiting
		//{
		//	if (NDIS_TEST_RECEIVE_CAN_PEND(ReceiveFlags))
		//	{
		//		ReturnFlags1 = 0;
		//		if (NDIS_TEST_RECEIVE_AT_DISPATCH_LEVEL(ReceiveFlags))
		//		{
		//			NDIS_SET_RETURN_FLAG(ReturnFlags1, NDIS_RETURN_FLAGS_DISPATCH_LEVEL);
		//		}

		//		NdisFReturnNetBufferLists(pFilter->FilterHandle, NetBufferLists, ReturnFlags1);
		//	}
		//}
		//else
		//{
